...
animateBars();
    ...
    $(document).ready(function () {
        ...
        $(".ae3-image-comparison").twentytwenty({
        ...
        // Mobile Menu Js
        // Disabled in React app: meanmenu re-parents/wraps nodes that React owns,
        ...
        // Venobox Video + Images
        function initVenobox() {
            // Destroy previous instance before re-init (important after cloned gallery items)
            if (window.__ae3Venobox && typeof window.__ae3Venobox.destroy === "function") {
                try {
                    window.__ae3Venobox.destroy();
                } catch (e) {
                    // ignore destroy errors from stale instances
                }
            }

            window.__ae3Venobox = new VenoBox({
        ...
        // Custom Cursor
        function customCursor(viewSelector = '.ae3-hover-view') {
        ...
        function gallaryScroll(){
            ...
            // Rebind lightbox so duplicated gallery anchors open correctly.
            initVenobox();
        }
        gallaryScroll();
	...
	sm.add("(min-width: 768px)", () => {
		if (document.querySelector("#ae3-smooth-wrapper") && document.querySelector("#ae3-smooth-content")) {
			ScrollSmoother.create({
				wrapper: "#ae3-smooth-wrapper",
				content: "#ae3-smooth-content",
				smooth: 1.8,
				effects: true,
				smoothTouch: 0.15,
